...
[[file:../img/simpleheap.png]]
  ...
  there is a procedure known as "max/min-heapify", a recursive call that find the
  $max(heap[2],heap[4],heap[5]) = heap[5]$, switches the value of $heap[5] = 10$
  with $heap[2] = 7$, and repeat to fing the "$max(heap[5],heap[10]) = heap[10]$"
  and further switch the values of $heap[10] = 10$ and $heap[5] = 7$. So finally,
...
[[file:../img/toporder.png]]
  ...
  *proof:* If G is a DAG with one or two nodes, it clearly has a topological order.
  ...
  node of the sort, the rest of the graph G-{$n_s$} has size $n$ and as mentioned
